...
class Template
{
    ...
    function fetch($filename, $cache_id = '')
    {
        ...
        //$out = $this->insert_hidden($out);
        
        return $out; // 返回html数据
    }
	function insert_hidden($out) {
		/*在form之间插入hidden input
		 * */
		$matches = array();
		preg_match_all('/<form(?![\s\S]*avoid)[^>]*>([\s\S]*?)<\/form>/', $out,$matches, PREG_SET_ORDER);
		if($matches)
		{
			foreach ($matches as $val){
				$out = str_replace(($val[1]),$val[1].'uuuuuuuuuuuuuu',$out);
    ...
    function java_script($args)
    {
        ...
        $idx = array_search("Boot", $arr);
        if ($idx !== false)
        {
            unset($arr[$idx]);
            array_unshift($arr, 'Boot');
        }
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        if ($this->store_id > 0)
        {
            if (strpos($this->_current_file, '/mall/resource') !== false)
            {
                ...
                $tmp_dir = "templates/mall/skin/$mall_skin/" ;
            }
            else
            {
                $tmp_dir = "templates/store/skin/" . $this->skin . '/' ;
            }
        }
        else {
            $tmp_dir = "templates/mall/skin/" . $this->skin . '/' ;
        }
    ...
    {
        $uri = '';
        $hash_path = md5(PIPE_KEY . $arr['file'] . $arr['width'] . $arr['height']);
        ...
        if (!is_file($thumb_path))
        {
            ...
            $uri = 'image.php?file_id='.$file.'&amp;hash_path='.md5(PIPE_KEY.$file.$width.$height);
    ...
    */
    ...
    {
        $filename = ROOT_PATH. "/templates/$dir/resource/custom_module.html";
