...
class PipeMysql
{
    ...
    function query($sql, $type = '', $times=0)
    {
        ...
        {   
            ...
            $error = mysql_error();
            /* 发生执行错误自动回滚 */
            if($this->_start_transaction)
            {
            	mysql_query('ROLLBACK',$this->_link_id) or $this->ErrorMsg();
            	//$this->commit();
            }
    ...
    function prefix($sql)
    {
        return preg_replace('/\s\`pipe_(.+?)\`([,|\s]?)/', ' ' .DB_PREFIX. '$1$2', $sql);
    }
    ...
    function start_transaction() {
    	$this->_start_transaction = 1;
    	mysql_query('start transaction',$this->_link_id) or exit('事务开始出错');
    }
    function commit()
    {
    	    	
    	if(mysql_query('commit',$this->_link_id) === false ) echo "提交事务出错!<br/>";
    	$this->_start_transaction = 0;
    ...
    function email_to_admin()
    {
    	...
    	file_put_contents('./data/sql.log', 'Error'.$error."  "."Errno:".$error_no.' 时间:'.date('Y-m-d h:i:s',time()));
    	//mail('784139247@qq.com','事务性sql错误','Error'.$error."  "."Errno:".$error_no.' 时间:'.date('Y-m-d h:i:s',time()));
    ...
    function ErrorMsg($message = '', $sql = '')
    {
        ...
        else
        {
            ...
            echo '<a href="http://www.daogoge.com/help/report.php?type=mysql&amp;dberrno=' . $error_no . '&amp;dberror=' . urlencode($error) . '" target="_blank">'. Lang::get('mysql_error_report') . '</a>';
        }
