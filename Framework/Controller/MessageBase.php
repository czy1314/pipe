...
function put_log($err, $msg, $file, $line)
{
    ...
    if (!is_dir('temp/logs'))
    {
        pipe_mkdir(ROOT_PATH . '/' . 'temp/logs');
    }
...
class Message extends MessageBase
{
    ...
    function report_link($err, $msg, $file, $line)
    {
        if (strncmp($msg, 'MySQL Error[', 12) == 0)
        {
            ...
            $url = 'http://zhaimao.daogoge.cn/help/faq.php?type=mysql&dberrno=' . $tmp_arr['dberrno'] . '&dberror=' .  urlencode($tmp_arr['dberror']);
        ...
        else
        {
            ...
            $url = 'index.php?app=issue&data=' . $str_report . '&amp;sign=' . md5($str_report . PIPE_KEY);
    ...
    function display()
    {
        ...
        else
        {
            ...
            $this->_config_seo('title', Lang::get('Boot_sysmsg'));
