...
class OrderPayment extends Object
{
	/**
	 * 
	...
	function  verify_status($order_info,$notify)
	{
		...
		switch ($notify['trade_status'])
		{
			...
			case 'TRADE_SUCCESS':               // 交易成功
				if ($order_info['status'] == WAIT_FIRST_PAY)
				{
			...
			case 'TRADE_CLOSED':                //回到等待首付支付状态
				$order_status = WAIT_FIRST_PAY;
				break;
	...
	function _change_order_status($id, $notify_result) {
	    ...
	    $order_type->respond_notify($id, $notify_result);    //响应通知
	}
}
