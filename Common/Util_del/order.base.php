...
class BaseOrder extends Object
{
    ...
    function respond_notify($order_id, $notify_result)
    {
        ...
        $model_order =& m('order');
        $one = $model_order->get("order_id={$order_id} and status !=".$notify_result['target']);
		if($one)
		{
			$where = "order_id = {$order_id}";
			$data = array('status' => $notify_result['target']);
			//echo $notify_result['target'].'ORDER_PENDING'.ORDER_PENDING.'|ORDER_ACCEPTED'.ORDER_ACCEPTED.'|ORDER_SHIPPED'.ORDER_SHIPPED.'|'.'ORDER_FINISHED'.ORDER_FINISHED.'|'.'ORDER_CANCLED'.ORDER_CANCLED;
			// ORDER_PENDING11|ORDER_ACCEPTED20|ORDER_SHIPPED30|ORDER_FINISHED40|ORDER_CANCLEDORDER_CANCLED
    ...
    {
        /* 默认都是等待实地认证 */
        $order_status = WAIT_REAL_IDENTIFY;
        /* 买家信息 */
        $visitor     =& env('visitor');
        ...
        return array(
            ...
            'type'          =>  isset($goods_info['type']) ? $goods_info['type'] : 'normal',
            ...
            'status'        =>  $order_status,
            'add_time'      =>  gmtime(),
        	'order_amount'  =>  $goods_info['firstpay'],
            'total_service'  => $goods_info['total_service'],
        	'total_price'  => $goods_info['total_price'],
        	'current_month'  => $goods_info['current_month'],
        	'current_month_pay'  => $goods_info['current_month_pay'],
        	'firstpay'  => $goods_info['firstpay'],
        	'sf_item'  => $goods_info['sf_item'],
        	'mon_fee'  => $goods_info['mon_fee'],
        	'mon_service'  => $goods_info['mon_service'],
        	'capital'  => $goods_info['capital'],        		
